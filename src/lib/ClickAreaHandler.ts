...
import type { Book, Page } from './Entity';
...
export const parseClick = (config: UIConfig) => (rect: Rect) => (
	clickPoint: Point,
	state: ReadingState
) => {
...
export const point2Command = (config: UIConfig) => (rect: Rect) => (clickPoint: Point) => {
	...
	if (50 - centerHalfWidth <= x && x <= 50 + centerHalfWidth) return Command.ViewState;
